...
const CashCountSection = ({
  ...
  const cashOut = (movements || [])
    ...
    .reduce((sum, m) => sum + Number(m.amount || 0), 0);
  const expected =
    opening + sales + cashIn - cashOut - refund - expense - credit;
  const diff =
    closeAmount === undefined || closeAmount === null
      ? 0
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        >
          GHS {Math.round(diff).toLocaleString()}
        </div>
      </div>
      {/* Show warning if difference is large */}
      {Math.abs(diff) >= 50 && (
        <div
          className={`mb-2 p-3 rounded border flex items-center gap-2 ${
            mode === "dark"
        ...
        >
          ...
          />
          <span>
            Warning: The difference (over/short) is large (GHS{" "}
            {Math.round(diff).toLocaleString()}). Please double-check your cash
            count and add a note.
          </span>
        </div>
      )}
      <div className="mb-4">
        ...
        >
          Closing Note{" "}
          {Math.round(diff) !== 0 ? (
            <span className="text-red-600">(required for over/short)</span>
          ) : null}
        </label>
        <textarea
      ...
      <button
        ...
        disabled={
          ...
          closeAmount === null ||
          (Math.round(diff) !== 0 && closeNote.trim() === "")
        }
