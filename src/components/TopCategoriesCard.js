...
export default function TopCategoriesCard({ selectedStore }) {
  ...
  const renderActiveShape = (props) => (
    <AnimatedActiveShape props={props} animatedOuterRadius={animatedOuterRadius} />
  );
      ...
      <div className="flex items-center justify-between border-b pb-2">
        <div className="flex items-center gap-2">
          <span className="bg-orange-50 p-2 rounded-full">
            <Icon icon="mdi:account-group" className="text-orange-500 text-lg" />
          </span>
          <span className="font-bold text-lg">Top Categories</span>
        </div>
        <a href="/categories" className="text-sm font-medium text-blue-900 hover:underline">View All</a>
      </div>
      <div className="flex flex-col items-center">
        {loading ? (
          <div className="py-8 text-center text-gray-400">Loading...</div>
        ) : topCategories.length === 0 ? (
          <div className="py-8 text-center text-gray-400">No category sales found</div>
        ) : (
          <>
            <div className="flex items-center w-full justify-center">
              <ResponsiveContainer width={220} height={220}>
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value, name) => [`${value.toLocaleString()} Sales`, name]}
                ...
                {topCategories.map((cat, idx) => (
                  <div key={cat.id} className="flex flex-col items-start mb-2">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`inline-block w-2 h-2 rounded-full`} style={{ background: COLORS[idx % COLORS.length] }}></span>
                      <span className="font-medium text-gray-900 text-md">{cat.name}</span>
                    </div>
                    <div className="flex items-baseline gap-1 pl-4">
                      <span className="font-bold text-xl text-gray-900">{cat.sales.toLocaleString()}</span>
            ...
            <div className="w-full mt-8">
              <div className="font-bold text-md text-gray-900 mb-2 px-2">Category Statistics</div>
              <div className="border rounded-xl px-4 py-3 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700 font-medium flex-1">Total Number Of Categories</span>
                  <span className="font-bold text-gray-900">{totalCategories.toLocaleString()}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="inline-block w-3 h-3 rounded-full bg-orange-500"></span>
                  <span className="text-gray-700 font-medium flex-1">Total Number Of Products</span>
                  <span className="font-bold text-gray-900">{totalProducts.toLocaleString()}</span>
                </div>
              </div>
            </div>
          </>
