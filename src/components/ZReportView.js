...
export default function ZReportView({
  ...
  if (!zReport)
    return (
      <div className="text-center py-8 text-gray-400">No Z-Report data.</div>
    );
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm dark:bg-gray-700">
            ...
            >
              {session.over_short !== null && session.over_short !== undefined
                ? `GHS ${Number(session.over_short).toLocaleString(undefined, {
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm dark:bg-gray-700">
            ...
            <div className="font-bold text-lg text-indigo-600">
              ...
              session.closing_cash !== undefined
                ? `GHS ${Number(session.closing_cash).toLocaleString(
                    undefined,
                    { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                  )}`
                : "N/A"}
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm dark:bg-gray-700">
            ...
            <div className="font-bold text-lg text-green-600">
              {zReport.totalSales !== null && zReport.totalSales !== undefined
                ? `GHS ${Number(zReport.totalSales).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}`
                : "N/A"}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-2">
            {paymentBreakdown && Object.keys(paymentBreakdown).length > 0 ? (
              Object.entries(paymentBreakdown).map(([type, amount]) => (
                <div
                  key={type}
                  className="bg-gray-50 rounded p-2 text-center dark:bg-gray-700"
                >
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm border border-gray-300 dark:border-gray-600">
              <thead className="bg-gray-100 dark:bg-gray-700">
                <tr>
                  <th className="px-4 py-2 border border-gray-300 text-gray-700 dark:border-gray-600 dark:text-gray-200">
                    Product
                  </th>
                  <th className="px-4 py-2 border border-gray-300 text-gray-700 dark:border-gray-600 dark:text-gray-200">
                    Quantity
              ...
              <tbody>
                {productsSold && productsSold.length > 0 ? (
                  productsSold.map((product, index) => (
                    <tr key={index}>
                      <td className="px-4 py-2 border border-gray-300 text-gray-900 dark:border-gray-600 dark:text-gray-200">
                        {product.name}
                      </td>
                      <td className="px-4 py-2 border text-center border-gray-300 text-gray-900 dark:border-gray-600 dark:text-gray-200">
                        {product.quantity}
                      </td>
                      <td className="px-4 py-2 border text-center border-gray-300 text-gray-900 dark:border-gray-600 dark:text-gray-200">
                        <div className="flex items-center justify-center gap-1">
                          <Icon
                            icon={
                              paymentIcons[product.paymentMethod] ||
                              paymentIcons.other
                            }
                            className="w-4 h-4 text-blue-600"
                          />
                          <span className="capitalize text-xs">
                            {product.paymentMethod === "momo"
                              ? "Mobile Money"
                              : product.paymentMethod === "mixed"
                              ? "Split Payment"
                              : product.paymentMethod || "Other"}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-2 border text-right border-gray-300 text-gray-900 dark:border-gray-600 dark:text-gray-200">
                        GHS{" "}
                  ...
                  <tr>
                    <td
                      className="px-4 py-2 border text-center border-gray-300 text-gray-500 dark:border-gray-600 dark:text-gray-400"
                      colSpan="4"
                    >
                      No products sold
                    </td>
