...
const HOURS = [
  "2 Am", "4 Am", "6 Am", "8 Am", "10 Am", "12 Am", "14 Pm", "16 Pm", "18 Am"
];
const HOUR_TO_INDEX = {2: 0, 4: 1, 6: 2, 8: 3, 10: 4, 12: 5, 14: 6, 16: 7, 18: 8};
    ...
    <div className="bg-white rounded-2xl shadow p-0 w-full max-w-xl">
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-32 bg-white border border-gray-200 rounded-md shadow-lg z-10">
              {PERIODS.map((p) => (
                <button
                  key={p}
                  className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${period === p ? "font-bold text-blue-600" : "text-gray-700"}`}
                  onClick={() => {
                    setPeriod(p);
      ...
      <div className="px-6 py-6">
        <div className="overflow-x-auto" ref={gridRef} style={{ position: "relative" }}>
          ...
          {HOURS.map((hour, i) => (
            <div key={hour} className="grid grid-cols-8 gap-[6px] items-center mb-1">
              <div className="text-xs font-semibold text-gray-500 text-right pr-2" style={{ minWidth: 36 }}>{hour}</div>
              {DAYS.map((_, j) => (
                <div
                  key={j}
              ...
              <div className="bg-white rounded-xl shadow-lg px-4 py-2 text-gray-900 text-base font-semibold flex flex-col items-center relative">
                ...
                <span className="absolute left-1/2 -bottom-2.5 -translate-x-1/2 w-4 h-4">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M8 16L0 8H16L8 16Z" fill="#fff" />
                    <path d="M8 16L0 8H16L8 16Z" fill="#e5e7eb" fillOpacity="0.2" />
                  </svg>
                </span>
              </div>
            </div>
