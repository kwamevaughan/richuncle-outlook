...
const Search = ({
  ...
  }, [isPopupOpen, onToggle]);

  // Handle external open trigger
  useEffect(() => {
    if (externalIsOpen && !isPopupOpen) {
      setAnimationState("entering");
      setIsPopupOpen(true);
    } else if (!externalIsOpen && isPopupOpen) {
      closePopup();
    }
  }, [externalIsOpen, isPopupOpen, closePopup]);
  ...
  const styles = useMemo(
    () => ({
      ...
      errorText: `text-sm font-medium ${
        mode === "dark" ? "text-red-400" : "text-red-700"
      }`,
  ...
  return (
    <>
      {/* Only show input field if not controlled externally */}
      {!externalIsOpen && onToggle === undefined && (
                  ...
                  {loading && (
                    <div className="space-y-4">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className={`animate-pulse p-4 rounded-2xl ${
                            mode === "dark" ? "bg-gray-700/30" : "bg-gray-100"
                          }`}
                        >
                          <div
                        ...
                        <div key={section} className="space-y-3">
                          <div className={styles.sectionHeader}>
                            <div className={styles.sectionIconContainer}>
                              <Icon
                                icon={
                                  SECTION_CONFIG[section.toLowerCase()]?.icon ||
                                  "lucide:folder"
                                }
                                className="h-4 w-4"
                              />
                            ...
                            {items.map((item) =>
                              section.toLowerCase() === "products" ? (
                                <div
                                  ...
                                  onClick={() => {
                                    router.push(
                                      `/products?viewProductId=${item.id}`
                                    );
                                    closePopup();
                                  }}
                                ...
                                <Link
                                  key={item.id}
                                  href={`${
                                    SECTION_CONFIG[section.toLowerCase()]?.href
                                  }/${item.id}`}
                                  onClick={closePopup}
                                >
